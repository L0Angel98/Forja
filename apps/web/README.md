...
- `e2e/` — Playwright. `playwright.config.ts` levanta apps/server y apps/web (build de producción, para que el service worker y el rendimiento se comporten como en real) vía `webServer`. Requiere una Postgres real migrada y sembrada (ver el job `e2e` en `.github/workflows/ci.yml`) — usa los usuarios de desarrollo de `packages/db/src/seed.ts`.
  ...
  - `reportar-offline.spec.ts` — dos tests. (1) Sin conexión, el reporte se guarda en IndexedDB y se muestra como pendiente. El "sin conexión" se simula forzando el `fetch` de `POST /api/fallas` a rechazar (`page.addInitScript`) en vez de `context.setOffline`: con el service worker de producción activo, `context.setOffline` deja el POST sin llegar al server pero el banner de pendiente tampoco aparecía en CI real — la emulación de red a nivel CDP no resolvía de forma fiable con el fetch hecho desde ese contexto. Usa un tag inventado ("PRE-03"): no hace falta un id real solo para verificar que se encola. (2) Reconexión sin duplicar (acceptance criteria de la spec): mismo mecanismo de fetch forzado, pero conmutable — se apaga y se dispara un evento `online` sintético para simular la reconexión sin recargar la página, y se verifica contra `GET /api/fallas` que el reporte llegó al server exactamente una vez. Este sí necesita un id de máquina real (no hay catálogo por HTTP hoy), así que lo lee directo de la Postgres sembrada (`e2e/db.ts`, usa `@forja/db` — única razón por la que `apps/web` depende de `@forja/db` en `devDependencies`, exclusivo de `e2e/`).
